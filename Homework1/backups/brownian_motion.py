...
m0 = e0 = sig0 =  1
M = 40*m0
v0 = np.sqrt(2*e0/m0) * 20
t0 = sig0 * np.sqrt(m0/2*e0) * 0.00005
dt = 0.01*t0
#nTimeSteps = int(round(100 * t0 / dt))
nTimeSteps = 10000
print(nTimeSteps)
l = 100 * sig0
...
class Particle:
	...
	def get_lennard_jones(self,particle,e0,sig0):
		x1,y1 = self.positions
		x2,y2 = particle.positions
		rx = x2 - x1
		ry = y2 - y1
...
# by implementing the leapfrog algorithm
def new_step(particles,e0,sig0):
	# Update the positions at a half time step
	xOlds = yOlds = []
	for i in range(len(particles)):
		# Retain old positions in case of boundary collision
		xOld,yOld = particles[i].positions
	...
	# Get the forces at a half time step
	forceList = []
	for i in range(len(particles)):
		totalForces = [0,0]
        # Get total force in each direction
		for j in range(len(particles)):
			if i != j:
				totalForces += np.asarray(particles[i].get_force_vectors(particles[j],e0,sig0))
		#totalForces = tuple(totalForces)
		forceList.append(tuple(totalForces))

	# Get the new velocities at full time step
	for i in range(len(particles)):
        # Get new velocities
		vx,vy = particles[i].get_new_velocities(forceList[i],m0,dt)
		particles[i].velocities = vx,vy

	# Get the new poisitions at a full time step
	for i in range(len(particles)):
		x,y = particles[i].get_position(dt/2)
		xOld = xOlds[i]
		yOld = yOlds[i]
